...
import "./moreFromCategory.scss"

interface Props {
    categoryName: string;
    categoryUrl: string;
    moreProductsFromCategory: Product[];
}

export const MoreFromCategory = (props: Props) => {
    const { categoryUrl, categoryName, moreProductsFromCategory } = props;
    ...
    return (
        <div>
            {categoryUrl && categoryName && <div>Ще з категорії <Link to={`/clothes/${categoryUrl}`} className="previu__nameBrand"><b>{categoryName}</b></Link></div>}
            <div className="previu">
                {moreProductsFromCategory.map(item => (
